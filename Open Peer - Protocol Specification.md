...
SCTP is an alternative TCP like peer-to-peer communication protocol that messages can be relayed over as an alternative to RUDP protocol should the underlying framework support SCTP.

### RUDP Protocol Specification

RUDP is a protocol that works like TCP over UDP which is negotiated with STUN packets. See the RUDP protocol specification.
...
For the mandatory "aes-cfb-32-16-16-sha1-md5" algorithm and encoded with a "agreement" where the remote public Diffie-Hellman key is not known in advance, the "keys" material will be empty and will only be sent when the remote side responds with the Diffie Hellman keying material. This means the sending side cannot send data until the remote side has sent Diffie Hellman keying material.


When the mandatory key is used, the AES CFB is initialized with these values and will continue to encrypt payloads using this keying until a new "0" package arrives. Once a new "0" package arrives all keying material is replaced and the algorithms with new keying information for subsequent messages are used for messages in a stream. The hmac integrity is calculated using the following algorithm, hmac(`<integrity-passphrase>`, "integrity:" + hex(hash(`<decrypted-message>`)) + ":" hex(`<iv>`)). The AES key and integrity passphrase remains the same until a new "0" package is sent, but the IV is changed for every message. The next IV used for the keying selected is calculated based upon the hash of the previous IV for the selected keying and previous hmac integrity value, next_iv = hash(hex(`<previous-iv>`) + ":" + hex(`<previous-integrity-hmac>`)).
